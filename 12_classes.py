'''
    Classes é uma estrutura que representa conjuntamente dados e 
    algoritmos. Uma classe pode ser comparada a uma "assadeira" com a
    qual se pode fazer diferentes "assados" (objetos), variando os 
    "ingredientes" (dados) e o "modo de fazer" (algoritmos). Apesar
    dessa variação, todos os objetos criados a partir da mesma classe
    terão sempre o formato determinado por esta.
'''
...
'''
    ESTRUTURA DE DADOS: é uma forma de organização dos dados de forma a
    otimizar o desenvolvimento e a execução de algoritmos que utilizam
    esses dados.

    Em geral, cada estrutura de dados resolve um problema específico.

    Formas de Estrutura de Dados:
    Pilha, Fila, Deque, Lista Duplamente Encadeada Grifo, Árvore Binária de Busca

    PILHA:
        Resolve o problema da inversão. Coloca o primeiro valor embaixo dos
        posteriores.

...
class DoublyLinkedList:
    ...
    def __init__(self):
        ...
        self.__count = 0        # Quantidade de elementos
    ...
    ''' Método privado que encontre e retorna o nodo da psoição especificada '''
    def __find_node(self, pos):
        ...
        # 3° caso: nodo em posição intermediária
    ...
    def insert(self, pos, val):
        ...
        # 1° caso: a lista está vazia e este é o primeiro nodo a ser inserido
        if(self.is_empty()):
    ...
    ''' Método que remove o nodo do posição especificada '''
    def remove(self, pos):
        ...
        # 2° caso: remoção do início da lista (posição 0)
        if pos == 0:
            # Será removido o primeiro nodo da lista
            removed = self.__head
            # O novo __head pass a ser o sucessor do nodo removido
        ...
        # 3° caso: remoção do final da lista (posição __count - 1)
        elif pos == self.__count - 1:
            # Será removido o último nodo da lista
            removed = self.__tail
            # O novo __tail for um nodo válido, ele não pode ter sucessor
            if self.__tail is not None: self.__tail.next = None
            # Em caso de remoção do último nodo restante da lista
        ...
        # 4° caso: remoção em posição intermediária
            ...
            # O nodo anteriror passa a apontar, à frente, para nada posterior
            before.next = after
            # O nodo posterior passa a apontar, para trás, para o nodo anterior
            after.prev = before

        # Decremento a quantidade de itens da lista
        self.__count -= 1
    ...
    ''' Método de atalho para remoção no final '''
    ...
    def peek(self, pos):
        # Verifica se aposição passada existe na lista

'''
    ...
    É uma estrutura de dados linear em que tanto a
    operação de inserção (push) quanto a operação de
    remoção (pop) acontecem no final (ou topo). Como
    conquência, o funcionamento da pilha obedece ao
    princípio LIFO (Last In, First Out): o último a
    entrar dever ser o primeiro a sair.
'''
...
class Stack:
    ...
    def pop(self):
        if self.is_empty():     # A pilha está vazia
            raise Exception('ERRO: impossivel remover de uma pilha vazia.')
    ...
    def peek(self):
        if self.is_empty():
            raise Exception('ERRO: impossivel consultar uma pilha vazia.')
